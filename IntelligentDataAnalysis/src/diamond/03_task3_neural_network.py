...
with open(REPORT_FILE, "w", encoding="utf-8") as f:
    ...
    if accuracy == 1.0:
        # Цей випадок залишаємо (хоча він малоймовірний)
        ...
        # Висновок, що враховує дисбаланс
        f.write(f"На відміну від Дерева рішень (яке показало 100% точність), Нейронна мережа **провалилася**.\n\n")
        f.write(f"Хоча загальна точність (accuracy) склала **{accuracy:.1%}**, це є ілюзією ('парадокс точності').\n\n")
        f.write("**Аналіз Матриці помилок та звіту по класах (`03_neural_network_metrics.txt`) показує:**\n")
        f.write("1. Модель **не навчилася розпізнавати** цільовий клас 'Investment Grade'.\n")
        f.write(f"2. Метрики `precision`, `recall` та `f1-score` для класу 'Investment Grade' **дорівнюють 0.00**.\n")
        f.write("3. Модель просто прогнозує 'Not Investment' для всіх зразків, отримуючи високу точність за рахунок **сильного дисбалансу класів** (966 'Not Investment' проти 34 'Investment Grade').\n\n")
        f.write("**Головний висновок:** Навіть на 1000 рядках даних, але при сильному дисбалансі класів, "
                "проста нейронна мережа (`MLPClassifier`) з параметрами за замовчуванням виявилася неефективною. "
                "Це доводить, що для таких задач Дерева рішень є набагато більш надійним інструментом.\n")
